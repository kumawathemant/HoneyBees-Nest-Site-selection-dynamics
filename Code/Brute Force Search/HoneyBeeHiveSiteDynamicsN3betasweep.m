%{
HoneyBeeHiveSiteDynamicsN3betasweep.m
...
% Loop over 
Bvals = -1:.2:1;
for bb1 = 1:length(Bvals)
for bb2 = 1:length(Bvals)
for bb3 = 1:length(Bvals)
B = [Bvals(bb1); Bvals(bb2); Bvals(bb3)];
...
title({'Bifurcation diagram','N=4',sprintf('B=[%f,%f,%f]',B(1),B(2),B(3))});
xlabel('Bifurcation paramter (u)','FontSize',16,'FontWeight','bold');
ylabel('Average Opinion (y)','FontSize',16,'FontWeight','bold');
hold off;

savefig(bifurcation_fig,sprintf('N3_Unfold_B_%d_%d_%d',bb1,bb2,bb3)) % Save figure to .fig file
save(sprintf('N3_Unfold_B_%d_%d_%d',bb1,bb2,bb3),'y') % Save data to .mat file
close(bifurcation_fig)
